...
**Status:** Complete (Phase 12.1-12.1.1) ✅
...
- Scalar: ~5ms (baseline)
- NEON (ARM64): ~309μs mean (16x faster, measured in integration tests)
- AVX2 (x86_64): ~40μs theoretical (30x faster)
...
### 6. Verifying SIMD in Production

**Automatic Detection:** Titan automatically detects CPU capabilities at runtime and selects the best SIMD instruction set (AVX2 > SSE2 > NEON > scalar).

**Configuration:** SIMD is enabled by default—no configuration required.

**Runtime Verification:**

Check Titan's startup logs for SIMD detection:

```bash
$ ./titan --config config.json
[INFO] WebSocket SIMD: Using AVX2 (32 bytes/cycle)
# or
[INFO] WebSocket SIMD: Using NEON (16 bytes/cycle)
# or
[INFO] WebSocket SIMD: Using SSE2 (16 bytes/cycle)
# or
[INFO] WebSocket SIMD: Using scalar fallback (CPU feature not available)
```

**CPU Requirements:**
- **AVX2**: Intel Haswell+ (2013), AMD Excavator+ (2015)
- **SSE2**: x86_64 (baseline, all 64-bit Intel/AMD)
- **NEON**: ARM64/AArch64 (baseline, all 64-bit ARM)
- **Scalar**: Any CPU (automatic fallback)

**Performance Monitoring:**

Use Prometheus metrics to track WebSocket frame processing time:

```bash
# Average frame unmask time (should be <1ms for 1MB frames with SIMD)
titan_websocket_frame_unmask_duration_seconds
```

If unmask duration is unexpectedly high (>1ms for 1MB), check:
1. CPU throttling (`cat /proc/cpuinfo | grep MHz`)
2. CPU governor (`cat /sys/devices/system/cpu/cpu*/cpufreq/scaling_governor`)
3. Container CPU limits (`docker stats`)
