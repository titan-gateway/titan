...
@pytest.fixture
def titan_config_tls(tmp_path, mock_backend_1, mock_backend_2):
    """Create a Titan configuration file with TLS enabled for testing"""
    config = {
        "version": "1.0",
        "server": {
            "worker_threads": 1,
            ...
            "backlog": 128,
            "tls_enabled": True,
            "tls_certificate_path": str(REPO_ROOT / "server.crt"),
            "tls_private_key_path": str(REPO_ROOT / "server.key"),
        },
        "upstreams": [
            {
                "name": "test_backend",
        ...
        "logging": {"level": "info", "format": "text"},
        "metrics": {"enabled": True, "port": 9091, "path": "/metrics"},
    }

    config_path = tmp_path / "titan_test_tls.json"
    with open(config_path, "w") as f:
        json.dump(config, f, indent=2)
...
def titan_server_tls(process_manager, mock_backend_1, mock_backend_2):
    ...
    # Verify TLS certificates exist (should be generated by CI or local setup)
    cert_path = REPO_ROOT / "server.crt"
    key_path = REPO_ROOT / "server.key"
    if not cert_path.exists() or not key_path.exists():
        raise RuntimeError(
            f"TLS certificates not found. Generate with:\n"
            f"openssl req -x509 -newkey rsa:2048 -nodes "
            f"-keyout server.key -out server.crt -days 1 "
            f"-subj '/C=US/ST=Test/L=Test/O=Titan/CN=localhost'"
        )
    ...
    import tempfile
    config_fd, config_path = tempfile.mkstemp(suffix=".json", prefix="titan_tls_")
    with os.fdopen(config_fd, "w") as f:
        json.dump(config, f, indent=2)

    # Start Titan with TLS
    proc = process_manager.start_process(
        "titan-tls",
    ...
    # Wait for Titan to be ready on 8443
    if not process_manager.wait_for_port(8443, timeout=10):
        stdout, stderr = proc.communicate(timeout=1)
        print(f"Titan TLS stdout: {stdout}")
        print(f"Titan TLS stderr: {stderr}")
        raise RuntimeError("Titan failed to start on port 8443 with TLS")
