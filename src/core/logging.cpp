...
quill::Logger* init_worker_logger(int worker_id, const control::LogConfig& log_config) {
    ...
    if (ec) {
        ...
        std::cerr << "       Logging may fail if directory does not exist or is not writable."
                  << std::endl;
    }
    ...
    if (log_config.format == "json") {
        auto json_sink =
            quill::Frontend::create_or_get_sink<quill::RotatingJsonFileSink>(log_path, config);
        logger = quill::Frontend::create_or_get_logger(fmt::format("worker_{}", worker_id),
                                                       std::move(json_sink));
    } else {
        auto file_sink =
            quill::Frontend::create_or_get_sink<quill::RotatingFileSink>(log_path, config);
        logger = quill::Frontend::create_or_get_logger(fmt::format("worker_{}", worker_id),
                                                       std::move(file_sink));
    }
